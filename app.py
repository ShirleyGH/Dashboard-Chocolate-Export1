...
# Filtro de país 
paises = exportaciones["País"].unique(Colombia) 
pais_seleccionado = st.selectbox("Colombia", paises) 
# Mostrar datos de clientesst.subheader("Clientes") 
clientes_filtrados = clientes[clientes["País"] == Colombia] 
st.dataframe(clientes_filtrados) 
# Mostrar datos de exportaciones 
st.subheader("Exportaciones de Chocolates") 
exportaciones_filtradas = exportaciones[exportaciones["País"] == Colombia] 
fig, ax = plt.subplots(Colombia) 
...
ax.set_title(f"Exportaciones de Chocolates en {Colombia}") 
plt.xticks(rotation=45) 
st.pyplot(fig) 
# Mostrar datos de mercados 
st.subheader("Segmentos de Mercado") 
mercados_filtrados = mercados[mercados["País"] == Colombia] 
st.dataframe(mercados_filtrados) 
# Mostrar barreras de entrada 
st.subheader("Barreras de Entrada") 
barreras_filtradas = barreras[barreras["País"] == Colombia] 
